...
def process_input(text, prior_context=None, mode="abduction"):
    """
    Process input text to generate an existential graph, output, guidance, and context in abduction mode.
    
    Args:
        text (str): User input text.
        prior_context (dict): Previous EG context, if any.
        mode (str): Fixed to "abduction" for hypothesis generation.
    
    Returns:
        tuple: (output, guidance, context, eg)
    """
    doc = nlp(text)
    ...
    hypothesized_signs = hypothesize_signs(dyn_interpretant, dyn_object)
    if hypothesized_signs:
        eg["first_cut"]["value"] = hypothesized_signs[0]  # Take first hypothesis
        eg["double_cut"]["value"]["immediate"] = initial_meaning(imm_object, hypothesized_signs[0], context)
...
def main():
    """
    Command-line interface for processing inputs in abduction mode.
    Prompts for text inputs, processes them, and displays outputs, guidance, and EGs.
    """
    prior_context = None
    print("Running in abduction mode. Enter text inputs to process (type 'quit' to exit).")
    ...
    while True:
        ...
        try:
            output, guidance, context, eg = process_input(text, prior_context, mode="abduction")
            print("\nOutput:", output)
            print("Guidance:", guidance)
            print("EG:", json.dumps(eg, indent=2))
            print("\n")
            prior_context = context  # Update context for next iteration
